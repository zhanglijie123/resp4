...
import cn.sunline.icore.dp.serv.type.ComDpFroze.DpForceDeductIn;
import cn.sunline.icore.dp.serv.type.ComDpFroze.DpForceDeductOut;
 ...
 *         </p>
 */
public class DpForceDeduct {

	private static final BizLog bizlog = BizLogUtil.getBizLog(DpForceDeduct.class);
	 ...
	 * @return 法院扣划支取输出接口
	 */
	public static DpForceDeductOut doMain(DpForceDeductIn cplIn) {
		...
		// 账户扣划
		DpForceDeductOut cplOut = acctDeduct(cplIn);

		// 扣划转出登记
		registerDeduct(cplIn, cplOut.getSub_acct_no(), cplOut.getAct_withdrawal_amt());
	 ...
	 * @return 账户扣划支取输出接口
	 */
	private static DpForceDeductOut acctDeduct(DpForceDeductIn cplIn) {

		bizlog.method(" DpForceDeduct.acctDraw begin >>>>>>>>>>>>>>>>");

		// 初始化输出接口
		DpForceDeductOut cplOut = BizUtil.getInstance(DpForceDeductOut.class);
		...
		if (accessOut.getDd_td_ind() == E_DEMANDORTIME.DEMAND) {// 活期子户
			...
			// 活期子户支取输入赋值
			demandDrawIn.setAcct_no(cplIn.getAcct_no()); // 账号
			demandDrawIn.setAcct_type(cplIn.getAcct_type()); // 账户类型
			demandDrawIn.setAcct_name(cplIn.getAcct_name()); // 账户名称
			demandDrawIn.setProd_id(cplIn.getProd_id()); // 产品编号
			demandDrawIn.setCcy_code(cplIn.getCcy_code()); // 交易币种
			demandDrawIn.setSub_acct_seq(cplIn.getSub_acct_seq());
			demandDrawIn.setBack_value_date(cplIn.getBack_value_date()); // 起息日期
			demandDrawIn.setCash_trxn_ind(E_CASHTRXN.TRXN); // 现转标志
			demandDrawIn.setRed_blue_word_ind(null); // 红蓝字记账标志
			demandDrawIn.setForce_draw_ind(cplIn.getForce_draw_ind()); // 是否强制借记
			...
			demandDrawIn.setTrxn_amt(cplIn.getTrxn_amt()); // 交易金额
			demandDrawIn.setFroze_no(cplIn.getFroze_no()); // 冻结编号
			demandDrawIn.setUnfroze_amt(cplIn.getUnfroze_amt()); // 解冻金额
			...
			cplOut.setAcct_no(cplDrawOut.getAcct_no()); // 账号
			cplOut.setAcct_name(cplDrawOut.getAcct_name()); // 账户名称
			cplOut.setCcy_code(cplDrawOut.getCcy_code());
			cplOut.setFroze_no(cplIn.getFroze_no());
			cplOut.setSub_acct_seq(accessOut.getSub_acct_seq());
			cplOut.setSub_acct_no(accessOut.getSub_acct_no());
			cplOut.setAct_withdrawal_amt(cplDrawOut.getAct_withdrawal_amt()); // 交易金额
		}
		else {// 定期子户
			...
			timeDrawIn.setAcct_no(cplIn.getAcct_no()); // 账号
			timeDrawIn.setAcct_type(cplIn.getAcct_type()); // 账户类型
			timeDrawIn.setAcct_name(cplIn.getAcct_name()); // 账户名称
			timeDrawIn.setSub_acct_seq(cplIn.getSub_acct_seq()); // 子账户序号
			timeDrawIn.setCcy_code(cplIn.getCcy_code()); // 货币代码
			timeDrawIn.setBack_value_date(cplIn.getBack_value_date()); // 起息日期
			...
			timeDrawIn.setTrxn_amt(cplIn.getTrxn_amt()); // 交易金额
			timeDrawIn.setFroze_no(cplIn.getFroze_no()); // 冻结编号
			timeDrawIn.setUnfroze_amt(cplIn.getUnfroze_amt()); // 解冻金额
			...
			timeDrawIn.setForce_draw_ind(cplIn.getForce_draw_ind()); // 是否强制借记
	 ...
	 *            实际扣划金额
	 */
	private static void registerDeduct(DpForceDeductIn cplIn, String subAcctNo, BigDecimal factDeductAmt) {
		...
		forceDeductInfo.setAcct_no(cplIn.getAcct_no());// 账号
		forceDeductInfo.setTrxn_amt(factDeductAmt);// 实际扣划金额
		...
		forceDeductInfo.setForce_deduct_reason(cplIn.getForce_deduct_reason());// 扣划原因
	 ...
	 *            法院扣划支取输入接口
	 */
	public static void checkMain(DpForceDeductIn cplIn) {
	 ...
	 *            法院扣划支取输入接口
	 */
	private static void checkInputData(DpForceDeductIn cplIn) {
		...
		// 扣划原因不能为空
		BizUtil.fieldNotNull(cplIn.getForce_deduct_reason(), DpDict.A.force_deduct_reason.getId(), DpDict.A.force_deduct_reason.getLongName());
	 ...
	 *            法院扣划输入
	 */
	private static void checkDeductAcct(DpForceDeductIn cplIn) {
		...
		if (accessOut.getDd_td_ind() == E_DEMANDORTIME.DEMAND) {
			...
			demandDrawIn.setSub_acct_seq(cplIn.getSub_acct_seq());// 子户序号
			demandDrawIn.setAcct_no(cplIn.getAcct_no());// 账号
			demandDrawIn.setAcct_type(cplIn.getAcct_type());// 账号类型
			demandDrawIn.setAcct_name(cplIn.getAcct_name());// 账号名称
			demandDrawIn.setProd_id(cplIn.getProd_id());// 产品编号
			demandDrawIn.setCcy_code(cplIn.getCcy_code());// 货币代号
			...
			demandDrawIn.setForce_draw_ind(cplIn.getForce_draw_ind());// 是否强制标志
			demandDrawIn.setCheck_password_ind(E_YESORNO.NO);// 验密标志
			demandDrawIn.setTrxn_password(null);// 交易密码
			...
			timeDrawIn.setCard_no(cplIn.getAcct_no());// 卡号
			timeDrawIn.setAcct_no(cplIn.getAcct_no());// 账号
			timeDrawIn.setAcct_type(cplIn.getAcct_type());// 账号类型
			timeDrawIn.setAcct_name(cplIn.getAcct_name());// 账号名称
			timeDrawIn.setSub_acct_seq(cplIn.getSub_acct_seq());// 子账号序号
			timeDrawIn.setCcy_code(cplIn.getCcy_code());// 货币代号
			timeDrawIn.setBack_value_date(cplIn.getBack_value_date());// 起息日期
			timeDrawIn.setCash_trxn_ind(E_CASHTRXN.TRXN);// 现转标志
			timeDrawIn.setCheck_password_ind(null);// 验密标志
			timeDrawIn.setTrxn_password(null);// 交易密码
			...
			timeDrawIn.setForce_draw_ind(cplIn.getForce_draw_ind());// 是否强制标志
